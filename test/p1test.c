...
int main() {
	...
	INIT_LIST_HEAD(&qa);
	if (!list_empty(&qa)) adderrbuf("list_empty(qa): unexpected FALSE   ");
	addokbuf("Inserting...   \n");
	for (i = 0; i < 10; i++) {
		if ((q = allocPcb()) == NULL)
	...
	if (list_empty(&qa)) adderrbuf("list_empty(qa): unexpected TRUE"   );
	...
	if (!list_empty(&qa))
		adderrbuf("list_empty(qa): unexpected FALSE   ");

	addokbuf("insertProcQ(), removeProcQ() and list_empty() ok   \n");

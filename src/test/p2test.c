...
pid_t p8pid, leaf1pid, leaf2pid, leaf3pid,
    leaf4pid; /* p8 and it's grandchilds' pids */

void p2(), p3(), p4(), p5(), p5a(), p5b(), p5c(), p6(), p7(), p7a(), p5prog(),
    p5mm();
...
void p3() {
  ...
  if (((usr_t2 - usr_t1) > MINCLOCKLOOP) ||
      ((glob_t2 - glob_t1) < MINCLOCKLOOP))
...
void p4() {
  ...
  if (p4inc == 3) {
    print(
        "error: second incarnation of p4 did not terminate with original p4\n");
    PANIC();
  }
...
void p5sys(unsigned int a1, unsigned int a2, unsigned int a3, unsigned int a4) {
  ...
  if (!p5status) {
    ...
    print("p5c - try to execute return value SYSCALL in user mode (it should "
          "be SYS11)\n");
  } else {
    if (a1 == SYSQUESTION) {
